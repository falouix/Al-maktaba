...
function AddNew(counter1) {
  ...
  const [title_status, setStatusTitle] = useState("");
  ...
  const createStatus = async (event) => {
    ...
    if (title_status === "" && !selectedFile) {
      alert("status est vide!")
      return;
    }

    try {
      await axios.post(Environment.api_url + "forum.php",
        {
          action: 'creat_status',
        ...
        .then(async res => {
          ...
          if (selectedFile != undefined) {
            axios.post(Environment.api_url + "forum.php?action=creat_status_image&insert_id=" + res.data.insert_id,
              formData, {
              headers: {
      ...
      <div className="container_copy">
        ...
        <div className="input-field_addnew">
          ...
          <textarea
            ...
            placeholder="Ecrire description de votre sujet"
            rows="6" />
        </div>
